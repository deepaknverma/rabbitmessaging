 ...
 * @Last Modified time: 2017-04-06 11:52:47
 */
...
class rabbit {
	...
	constructor(options) {
		...
		this.url = (options.url) ? options.url : false;											// URL to the server
		this.prefetch = (options.prefetch) ? options.prefetch : 10;								// Number of channels allowed in a single connection
		this.heartbeat = options.heartbeat ? options.heartbeat : 10;							// number of sec after which server send back a connection alive heartbeat else throws an error
		...
		this.maxRetry = (options.maxRetry) ? options.maxRetry : 60;								// number times the rabbit connection retry after failure. that calculate to 10 mins of retry before service fails						 																		// Number of retry connection count
		this.xMessageTtl = (options.xMessageTTL) ? options.xMessageTtl : 60000;					// Time to live for a return queue
		this.xExpires = (options.xExpires) ? options.xExpires : 60000;							// Expiry time for for a queue
		this.XMaxPriority = (options.XMaxPriority) ? options.XMaxPriority : 10;					// Priority for this queue
		this.noAck = (options.noAck) ? options.noAck : false;									// if true, the broker won't expect an acknowledgement of messages delivered to this consumer
		...
		return new Promise(function(success, fail) {
			amqp.connect(self.url + '?heartbeat=' + self.heartbeat, self.socketOptions)
				.then(function(conn) {
					...
					self.conn.on('blocked', function(reason) {
					...
					self.conn.on('close', function(err) {
						...
						// (err !== 'undefined' && require('amqplib/lib/connection').isFatalError(err) === true) ? self.__reconnect(self) : false;
						self.conn.status = false;
	...
	send(queues, msg, options) {
		...
		return new Promise(function(success, fail) {
			...
			ok = self.ch.assertExchange((options.exchange) ? options.exchange : 'PIH', (options.routing) ? options.routing : 'direct', options);
		...
		return new Promise(function(success, fail) {
			...
			ok = ok.then(function() {
				...
				function thisPayload(msg) {
					...
					function reply(returnError, returnMessage) {
						let result = {};
	...
	subscribe(topic, options, confirm) {
		...
		return new Promise(function(success, fail) {
			...
			// Check for Exchange and if not created then go ahead and create it
			ok = self.ch.assertExchange((options.exchange) ? options.exchange : 'PIH', (options.routing) ? options.routing : 'fanout', options);
			...
			ok = ok.then(function(qok) {
				// console.log(qok);
				return self.ch.bindQueue(queue, (options.exchange) ? options.exchange : 'PIH', topic, options)
					.then(function() {
						return queue;
					});
			});
	...
	publish(msg, options) {
		...
		return new Promise(function(success, fail) {
			...
			ok = self.ch.assertExchange((options.exchange) ? options.exchange : 'PIH', (options.routing) ? options.routing : 'fanout', options);
			ok.then(function() {
				self.ch.publish((options.exchange) ? options.exchange : 'PIH', (options.routingKey) ? options.routingKey : 'fanout', self._bufferify(msg), options);
	...
	publishToQueue(queues, message, method, options) {
		...
		return new Promise(function(success, fail) {
			...
			if (!queues) {
				return fail(errs.create({
					title: 'rabbitMessagingError',
					message: 'Unable to publish to queue. No queue provided.',
					code: 500
				}));
			}
			...
			if (!message) {
				return fail(errs.create({
					title: 'rabbitMessagingError',
					message: 'Unable to publish to queue. No message provided.',
					code: 500
				}));
			}
			// lets push to queue
